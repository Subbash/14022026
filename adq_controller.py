...
def _get_bias_setter(dev):
    for name in ("SetAdjustableBias", "ADQ_SetAdjustableBias"):
        if hasattr(dev, name):
            return getattr(dev, name), name
    return None, None


def _get_bias_getter(dev):
    for name in ("GetAdjustableBias", "ADQ_GetAdjustableBias"):
        if hasattr(dev, name):
            return getattr(dev, name), name
    return None, None
...
def adq_apply_adjustable_bias(
    ...
    set_bias, set_bias_name = _get_bias_setter(dev)
    if set_bias is None:
        raise RuntimeError(
            "This pyadq/ADQAPI build does not expose SetAdjustableBias(channel, codes). "
            "Tried SetAdjustableBias and ADQ_SetAdjustableBias."
        )

    get_bias, get_bias_name = _get_bias_getter(dev)
    results = []
    ...
    for channel in channels:
        ...
        status = set_bias(channel, bias_codes)
        ...
        print(
            ...
            f"setter={set_bias_name}, getter={get_bias_name}"
        )
        results.append(
            {
                "channel": channel,
